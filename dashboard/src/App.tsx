...
const DEMO_MODE = true; // TokenGate disabled - Cognito auth handles access control
...
function Dashboard({ accessToken }: { accessToken: string }) {
  ...
  // Load graph from local path (fallback for demo/local mode)
  useEffect(() => {
    if (currentProjectId) return; // Already loaded via project selector
    ...
    <div className="h-full w-full flex flex-col bg-root text-text-primary noise-overlay">
      ...
      <header className="flex items-center px-3 sm:px-5 py-3 bg-surface border-b border-border-subtle shrink-0 gap-2 sm:gap-4">
        ...
        <div className="flex items-center gap-3 sm:gap-5 shrink-0 min-w-0">
          ...
          <ProjectSelector currentProjectId={currentProjectId} onSelect={onProjectSelect} />
          <div className="w-px h-5 bg-border-subtle hidden sm:block" />
          <PersonaSelector />
